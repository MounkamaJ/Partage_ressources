...
def partage(client_socket, filename):
    ...
    with open('LOGS/log.txt', 'a') as logs_file:
        ...
        logs_file.write(f"{current_time} - Fichier {filename} partagé: \n")
            ...
            with open(save_path, 'wb') as file:
                while True:
                    ...
                    if data == b"END": # Vérifier si le message de fin est reçu
                        ...
                        progress['value'] = (totalrecv / filesize) * 100
                    ...
                    if totalrecv >= filesize: # Vérifier si la taille du fichier est atteinte
                        ...
                        with open('LOGS/log.txt', 'a') as logs_file:
                            current_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                            logs_file.write(f"{current_time} - Fichier {filename} téléchargé: \n")
